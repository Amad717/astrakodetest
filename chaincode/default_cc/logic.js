...
class testContract extends Contract {
  ...
  async getMarks(ctx, studentId, subject) {
    ...
    if (subject != null) {
      if (marks[subject] == null) {
        throw new Error('Student %s has no resgistered marks for subject %s',
                        studentId, subject);
      }
  ...
  async getAvgMarks(ctx, studentId, subject) {
    ...
    for (let subject in marks) {
      let sum = 0
      marks[subject].forEach(t => { sum += Number(t) })
  ...
  async deleteMarks(ctx,studentId) {
    ...
    console.log('Marks belonging to student %s were deleted from the ledger',
                studentId);
