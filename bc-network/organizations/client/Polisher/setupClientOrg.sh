...
function setupOrdererOrg {
  ...
  [ $LOG_LEVEL -ge 4 ] && set -x
  fabric-ca-client enroll -u https://caAdmin:caAdminpw@$CA_HOST:$CA_PORT\
      --caname $CA_NAME --tls.certfiles $FABRIC_CA_SERVER_HOME/tls-cert.pem --loglevel "${FABRIC_LOGS[$LOG_LEVEL]}"
  [ $LOG_LEVEL -ge 4 ] && set +x
  ...
  do
    ...
    [ $LOG_LEVEL -ge 4 ] && echo
    [ $LOG_LEVEL -ge 4 ] && set -x
    fabric-ca-client register --caname "$CA_NAME" --id.name "client$I" --id.secret client"$I"pw\
        --id.type client --tls.certfiles $FABRIC_CA_SERVER_HOME/tls-cert.pem --loglevel "${FABRIC_LOGS[$LOG_LEVEL]}"
    [ $LOG_LEVEL -ge 4 ] && set +x
    mkdir -p $NODE_PATH
