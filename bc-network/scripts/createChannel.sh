...
createChannel() {
  setGlobals
	...
	while [ $rc -ne 0 -a $COUNTER -lt $MAX_RETRY ] ; do
      ...
      -f "$ROOT/channel-artifacts/$CHANNEL_NAME.tx" --outputBlock "$ROOT/channel-artifacts/$CHANNEL_NAME.block" --tls\
...
## Join all the clients to the channel
for ((I = 1; I <= "$NUM_MEMBERS"; I++)); do
  NODE='CHANNEL_MEMBER'"$I"'_NODE'
  NODE="${!NODE}"
  ORG='CHANNEL_MEMBER'"$I"'_ORG'
  ORG="${!ORG}"
